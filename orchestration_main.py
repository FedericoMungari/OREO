...
def main():
    ...
    parser.add_argument("-c", "--config", action = "store", required = False, type = str,
                        help = "Configuration file containing the orchestration scenario")
    ...
    if args.config is None:
        error_message = "Error - no configuration file given."
        print(error_message)
    ...
    services_L = config["services_L"]
    services_Q = config["services_Q"]
    services_P = config["services_P"]
    service_freq = config["service_freq"]
    functions = config["functions"]
    functions_compl = config["functions_compl"]
    services_conf = config["services_conf"]
    resource = config["resource"]
    budget = config["budget"]
    xApp_mem_req = config["xApp_mem_req"]
    theta = config["theta"]
    semantic = config["semantic"]
    services_conf_graph = config["services_conf_graph"]
    service_output_quality = config["service_output_quality"]

    services, services_L, services_Q, services_P, service_freq, functions, functions_compl, services_conf, resource, budget, \
    xApp_mem_req, theta, semantic, services_conf_graph, service_output_quality, N_services, max_num_servconf, \
    N_functions, max_n_functions_per_s, max_num_compl, priority_lvls, cs_to_s, cs_list, services_conf_graph_output, \
    services_conf_graph_former, semantic_cs, lambda_semantic, f_multiplier, f_multiplier_c, quality_mapping_x, \
    quality_mapping_q = process_custom_scenario(services, services_L, services_Q, services_P, service_freq, functions,
                                                functions_compl, services_conf, resource, budget, xApp_mem_req, theta,
                                                semantic, services_conf_graph, service_output_quality)
...
if __name__ == '__main__':
    ...
    best_z_UB, best_v_UB, best_rho_UB = main()

    services_printing = [k[0]+" implemented with service configuration "+k[1] for k,v in best_z_UB.items() if v>0.5]
    xApp_printing = [str(k[3]) + "-th instance of function "+k[1]+" implemented with complexity factor " + str(k[2]) + " serving service "+k[0] for k, v in best_v_UB.items( ) if v>0.5]
    resource_printing = [str(k[2]) + "-th instance of function "+k[0]+" implemented with complexity factor " + str(k[1]) + " receives "+str(v)+" resourcs of "+k[3] for k, v in best_rho_UB.items( ) if v>0.]

    for msg in services_printing:
        print(msg)

    print("")

    for msg in xApp_printing:
        print(msg)

    print("")

    for msg in resource_printing:
        print(msg)
